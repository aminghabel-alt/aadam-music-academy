...
document.getElementById('login-pass').addEventListener('keydown',e=>{if(e.key==='Enter')doLogin();});/* SUPABASE */
const SUPABASE_URL='https://gqunxatjbifdhlyvwmuf.supabase.co';
const SUPABASE_KEY='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdxdW54YXRqYmlmZGhseXZ3bXVmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc5MTgxNDIsImV4cCI6MjA5MzQ5NDE0Mn0.KWMRnxGhavdW-i7OQKndktrshAI3FNn6CXRYHzgWQE4';
const sb=supabase.createClient(SUPABASE_URL,SUPABASE_KEY);
window.addEventListener('DOMContentLoaded',async()=>{
...
function setupUserUI(){
  ...
  if(u.role==='teacher'){
    document.getElementById('teacher-dashboard').style.display='block';
    document.getElementById('student-dashboard').style.display='none';
    document.getElementById('dash-greeting').textContent='سلام استاد '+u.name.split(' ')[0]+' 👋';
    // کد دعوت
    const inviteCard=document.getElementById('invite-card');
    const inviteDisplay=document.getElementById('invite-code-display');
    if(inviteCard&&u.invite_code){inviteCard.style.display='block';inviteDisplay.textContent=u.invite_code;}
    loadStudents();
    loadScoreStudents();
    loadTeacherDashboard();
    loadRecentScores();
...
async function saveScore(){
  ...
  const tech    = +document.getElementById('sc-tech').value   || 0;
  const rhythm  = +document.getElementById('sc-rhythm').value || 0;
  const melody  = +document.getElementById('sc-melody').value || 0;
  const fret    = +document.getElementById('sc-fret').value   || 0;
  const ear     = +document.getElementById('sc-ear').value    || 0;
  const comment = document.getElementById('sc-comment').value || '';
  const average = +((tech+rhythm+melody+fret+ear)/5).toFixed(1);
  ...
  showNotif('✅ نمره جلسه '+session+' برای '+studentName+' ثبت شد — میانگین: '+average);
  loadRecentScores();
}


async function loadTeacherDashboard(){
  // هنرجوهای فعال (جدول students)
  const {data:activeStudents} = await sb.from('students')
    .select('id,name,profile_id')
    .eq('teacher_id',currentUser.id)
    .neq('status','inactive');

  // هنرجوهای pending (از profiles که teacher_id دارن ولی هنوز توی students نیستن)
  const {data:profileStudents} = await sb.from('profiles')
    .select('id,name')
    .eq('teacher_id', currentUser.id)
    .eq('role','student');

  const activeProfileIds = new Set((activeStudents||[]).map(s=>s.profile_id).filter(Boolean));
  const pendingStudents = (profileStudents||[]).filter(p => !activeProfileIds.has(p.id));

  // همه هنرجوها برای select (فعال + pending)
  const allForSelect = [
    ...(activeStudents||[]).map(s=>({id:s.id, name:s.name, type:'active'})),
    ...(pendingStudents||[]).map(s=>({id:s.id, name:s.name+' (در انتظار)', type:'pending'})),
  ];

  const {data:scores} = await sb.from('scores').select('average').eq('teacher_id',currentUser.id);

  const totalStudents = (activeStudents||[]).length;
  const totalPending  = pendingStudents.length;
  const totalScores   = (scores||[]).length;
  const avgScore = totalScores > 0
    ? ((scores||[]).reduce((s,r)=>s+(+r.average||0),0)/totalScores).toFixed(1) : '—';

  document.getElementById('t-total-students').textContent = totalStudents;
  document.getElementById('t-total-scores').textContent   = totalScores + (totalPending>0 ? ` (+${totalPending} pending)`:'');
